...
class Entity(Object):
    def __init__(self,x,y,display,image,arm):
        ...
        #self.scal_image = pygame.transform.scale(self.image,(entity_x_scale,entity_y_scale))
        #self.rot_image = self.scal_image
        #self.rect = self.rot_image.get_rect(center = (round(self.pos.x),round(self.pos.y)))
        #self.hitbox = self.scal_image.get_rect(center = (round(self.pos.x),round(self.pos.y)))
        self.arm = arm
...
class Skeleton(Enemy):
    ...
    def update(self,p,tiles,dist,c_group,s_group,b_group,e,offset):
        self.dirty = 1
        self.die(p,c_group,s_group)
        self.move(p,tiles,dist,e)
    ...
    def move_to_player(self,p,tiles,dist,e):
    ...
    def chase_player(self,p,tiles,dist,e):
        dx,dy = self.pos.x - p.pos.x, self.pos.y - p.pos.y
        self.angle = math.degrees(math.atan2(dx,dy))
        self.rotate()
        self.move_to_player(p,tiles,dist,e)

    def move(self,p,tiles,dist,e):
        if dist <= self.sight:
            self.idle = False
            self.chase_player(p,tiles,dist,e)
        else:
            self.idle = True
            self.idle_move(tiles,p,e)
...
class Zombie(Enemy):
    ...
    def move_to_player(self,p,tiles,e):#change to a better movement
        if (p.pos.x + 3) >= self.pos.x and (p.pos.x - 3) <= self.pos.x:
            self.xvel = 0
        elif (p.pos.x - 3) > self.pos.x:
            self.xvel = 3
    ...
    def chase_player(self,p,tiles,e):
        dx,dy = self.pos.x - p.pos.x, self.pos.y - p.pos.y
        self.angle = math.degrees(math.atan2(dx,dy))
        self.rotate()
        self.move_to_player(p,tiles,e)

    def move(self,p,tiles,dist,e):
        if dist <= self.sight:
            self.idle = False
            self.chase_player(p,tiles,e)
        else:
            self.idle = True
            self.idle_move(tiles,p,e)
...
class Player(Entity):
    ...
    def update(self,moves,tiles,mx,my,b_group,f_group,e_group,s_group):
        self.change_lvl = False
        #self.die()
        self.rotate_to_mouse(mx,my)
        self.move(moves,tiles,f_group)
    ...
    def tile_collisions(self,tiles,moves,f_group):#not done abs() on mud + snow(when going diagonally)
        for t in tiles:
            ...
            elif t.t_type == "portal" and t.timer == 90:
                if t.rect_1.colliderect(self.hitbox):
...
#need to add for when snow and mud are next to each other
    ...
    def move(self,moves,tiles,f_group):
        self.xvel = 4
        self.yvel = 4
    ...
    def render_text(self):
        ...
        energy.draw(str(self.energyval)+"/"+str(self.energymax))
        health.draw(str(self.health)+"/"+str(self.healthmax))
    ...
    def attack(self,mx,my,b_group,e_group,s_group):
        if self.weapon == 0:
    ...
    def render(self):
        self.display.blit(self.rot_arm,self.arm_rect)#order matters as arms look weird if drawn over head
        self.display.blit(self.rot_image,self.rect)
        self.render_bars()
        self.render_text()
        self.render_hearts()
...
class Bullet(Object):
    ...
    def update(self,tiles,mx,my,player,enemies,spawners):
        self.move(tiles,mx,my,player,enemies,spawners)
        self.stop()
        #self.render()

    def move(self,tiles,mx,my,player,enemies,spawners):
